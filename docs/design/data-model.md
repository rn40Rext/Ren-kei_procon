...
**決定（2026-09-18）: プッシュ通知（FCM等）は導入せず、アプリ内通知（[#43](../../../../issues/43) [#44](../../../../issues/44)、`users/{uid}/notifications`）のみとする。** デバイストークンの保存スキーマ（issue が挙げていた `users/{uid}/devices/{token}`）は作らない。

理由:

1. **2026-09-15 にネイティブアプリを作らずWeb版に一本化する決定をした**（[ai-basic-motion.md 3章](ai-basic-motion.md)）ため、issue #45 が前提にしていた「iOS は Apple Developer Program が必要」というネイティブ push の懸念はそもそも無くなった。代わりに **Web Push 固有の制約**が問題になる
2. **iOS Safari は「ホーム画面に追加」でPWA化しない限りWeb Pushを一切受信できない**（通常のブラウザタブでは技術的に不可能）。一方、カメラ（`getUserMedia`）がiOSのPWA standaloneモードで不安定になる既知の不具合があるため、U-02（踊り解析）の信頼性を優先して「ホーム画面に追加はしない」運用を推奨している。**両立しない**
3. Android（Chrome）は通常のタブのままでも動くが、iOSと体験が非対称になる
4. #44 で実装済みのアプリ内通知（ベルアイコン・未読バッジ・一覧画面）で、アプリを開いたときに新着が分かるという最低限の要件は満たしている
5. issue #45 自身も「プロトタイプ/コンテスト段階ではアプリ内通知のみで十分な場合がある。不要と判断したらその旨を明記してクローズしてよい」としている

**再検討する条件**: 実機配布やAndroid限定運用など、上記2の制約が変わる状況になった場合。
...
### 6.3 `Users` → `users`（N-6・#39 で決定）

`Users/{uid}` に保護対象の実データは存在しない（プロトタイプ段階のテストデータのみ、チームで確認済み）ため、**移行スクリプトは作成せず、`Users` コレクションを破棄して `users` を新規に作り直す**方針とした（`videos`/`posts` 分離時の #57 と同じ判断）。`role` フィールドが無いドキュメントを変換で救う価値も無いため、この判断が最も単純で安全。
