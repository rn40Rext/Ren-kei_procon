...
Word 原本 `Ren-Kei_システム仕様書_基本設計書_v0.3.docx` から生成した**正典**です。ここを書き換えると原本と食い違い、正典が失われます。
...
**仕様と実装が食い違っている場合、勝手にどちらかへ寄せないでください。** [status/gap-analysis.md](status/gap-analysis.md) に差分として記録し、人間に判断を仰ぎます。

## ディレクトリの役割

| パス | 役割 | 編集 |
| --- | --- | --- |
| `spec/` | 製品仕様書 v0.3（正典） | **✗ 禁止** |
...
`spec/`（単数）が正典、`specs/`（複数）が作業単位です。紛らわしいので注意してください。
