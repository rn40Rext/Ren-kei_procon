...
export async function addComment(
  postId: string,
  // renId: type:'instructor'（師匠の教え）の場合のみ指定する。「どの連の管理者
  // としての発言か」をfirestore.rulesが検証するため、連管理者でなければ拒否される。
...
/** 特定ユーザーの投稿を新しい順に取得する（連の参加申請の申請者確認で使う。複合インデックス userId + createdAt）。 */
export async function fetchPostsByUser(userId: string, max: number): Promise<PostDoc[]> {
...
/** 投稿に指導者コメント(type:'instructor')が1件でもあるか（連管理者画面の「未アドバイス優先」並び替えで使う）。 */
